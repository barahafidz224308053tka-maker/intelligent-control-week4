...
if __name__ == "__main__":
    env = gym.make("MountainCar-v0", render_mode="human")  # tampilkan animasi
    ...
    for e in range(episodes):
        ...
        for time in range(200):  # max step MountainCar-v0
            action = agent.act(state)
            next_state, reward, terminated, truncated, _ = env.step(action)
            done = terminated or truncated

            # ubah reward agar lebih stabil (reward shaping)
